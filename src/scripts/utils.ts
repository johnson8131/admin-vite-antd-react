...
const loginAddress = '/user/login'; //window.g_config
 ...
 */
export const setStorage = <T>(key: string, value: T, isReplace: boolean = false) => {
...
export const getStorage = (key: string): IKeyStringProps | undefined => Rstore.get(key);
...
export const splitPathList = (pathname: string = ''): string[] => {
  ...
  return filterPathList.map((_, key) => `/${filterPathList.slice(0, key + 1).join('/')}`);
};
...
export const getAllRouteList = (
  ...
  const loopFindPath = (routeList: IRouteItemMinor[], value: IRouteItem[] = []) => {
    return routeList
...
const findItemRoute = (
  ...
  (Array.isArray(routeList) ? getAllRouteList(routeList) : allRouteInfo).find((item) =>
...
export const authRoutePage = (
  ...
  const { authority } = typeof values === 'string' ? findItemRoute(values) : values;
  return Array.isArray(authority) ? authority.includes(currentUser) : true;
};
...
export const getSearchValues = <T>(
  ...
  return typeof key === 'string' ? values[key] || '' : values;
};
...
export const findItem = <T = {}, K = string>(
  ...
  data.find((v) =>
    typeof v === 'object' ? Object.is(v[key], value) : Object.is(v, value),
  ) || {};
...
export const findItemIndex = <T = {}, K = string>(
  ...
  data.findIndex((v) =>
    typeof v === 'string' ? Object.is(v, value) : Object.is(v[key], value),
  );
 ...
 * @param {number} num
 */
export const numToThousands = (num: number) => (isNaN(+num) ? '' : num.toLocaleString());
...
export const filterItem = <T = {}, K = string>(option: {
  ...
  return data.filter((v) => {
    ...
    return !newValue.includes(typeof v === 'object' ? v[key] : v);
  });
};
...
export const toHorizontalLine = (str: any = '') =>
  typeof str === 'string' ? str.replace(/([A-Z])/g, '-$1').toLowerCase() : str;
