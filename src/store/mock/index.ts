import { assignAreaData, deepCopy } from '@scripts/utils';
...
export const asyncLoadAreaFile = (level?: 'provinces' | 'cities' | 'areas') => {
  return new Promise<TSetKeyStringProps>((resolve): any => {
    ...
    switch (level) {
      case 'provinces':
        return loadAfterCallback(import('@store/mock/china-area/provinces.json'));
      case 'cities':
      ...
      default:
        Promise.all([
          ...
          provincesData = assignAreaData(provincesData, citiesData, 'provinceCode');
