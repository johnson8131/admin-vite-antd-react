import { countNoticeNum, findItemIndex } from '@scripts/utils';
...
export default {
  ...
  effects: {
    *selectInboxItem({ payload }, { select, put }) {
      ...
      if (!(typeof _direct === 'number')) return;
    ...
    *selectOrStarItem({ payload }, { select, put }) {
      // 选中单个条目切换,标记星标切换
      const { dataSource: oldDataSource, noticeData: oldNoticeData } = yield select(
        ({ inbox }) => inbox,
      );
      let dataSource = oldDataSource;
